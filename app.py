...
def fmt_rp(v):
    ...
    a = abs(v)
    if a >= 1_000_000_000:
        return f"{sign}Rp {a/1_000_000_000:.2f} m"
    if a >= 1_000_000:
        return f"{sign}Rp {a/1_000_000:.1f} jt"
    if a >= 1_000:
...
.ctrl-card{margin-top:10px;padding:9px 10px}.ctrl-head{display:grid;grid-template-columns:1fr auto;gap:8px;align-items:start;margin-bottom:7px}.ctrl-title-wrap{display:flex;align-items:baseline;gap:8px;min-width:0}.ctrl-title-wrap .period{font-size:7.6px;color:#a9bfd6;white-space:nowrap}.ctrl-head .toggle{justify-self:end}
.toggle{display:grid;grid-template-columns:auto auto;border:1px solid rgba(73,128,177,.55);border-radius:5px;overflow:hidden;height:22px;min-width:116px}
.toggle button{border:0;background:#081d35;color:#dbe9f5;font-size:7px;padding:0 10px;cursor:pointer;white-space:nowrap}.toggle button.active{background:linear-gradient(180deg,#1d7cff,#1768e6);color:#fff}
...
function rp(v){const s=v<0?"-":"",a=Math.abs(v||0);if(a>=1e9)return `${s}Rp ${(a/1e9).toFixed(2)} m`;if(a>=1e6)return `${s}Rp ${(a/1e6).toFixed(1)} jt`;if(a>=1e3)return `${s}Rp ${(a/1e3).toFixed(1)} k`;return `${s}Rp ${Math.round(a).toLocaleString()}`;}
...
with st.popover("🔐 Admin"):
    ...
    configured_password = st.secrets.get("ADMIN_PASSWORD", "R3n0")
